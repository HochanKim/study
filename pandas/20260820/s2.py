import pandas as pd
from pathlib import Path

BASE = Path(__file__).parent
DATA = BASE / "data"
DATA.mkdir(exist_ok=True)

# 이번에는 일부러 빈 칸을 섞어 두었습니다. 실무 데이터처럼
sales_file = DATA / "sales2.csv"

rows = [
    "주문일,지점,사원,분류,상품,수량,단가",
    "2026-03-02,강남,김철수,가전,노트북,3,1200000",
    "2026-03-02,강남,김철수,주변기기,마우스,10,25000",
    "2026-03-03,홍대,이영희,가전,모니터,4,350000",
    "2026-03-03,부산,박민수,주변기기,키보드,8,45000",
    "2026-03-04,강남,김철수,가전,노트북,2,1200000",
    "2026-03-04,홍대,,주변기기,마우스,15,25000",
    "2026-03-05,부산,박민수,가전,모니터,,350000",
    "2026-03-05,강남,최지은,주변기기,키보드,12,45000",
    "2026-03-06,홍대,이영희,가전,노트북,1,1200000",
    "2026-03-06,부산,박민수,주변기기,마우스,20,25000",
    "2026-03-09,강남,최지은,가전,모니터,5,350000",
    "2026-03-09,홍대,정하늘,주변기기,키보드,6,",
    "2026-03-10,부산,박민수,가전,노트북,2,1200000",
    "2026-03-10,강남,김철수,주변기기,마우스,7,25000",
]

# with open(sales_file, "w", encoding="utf-8") as f:
#     for row in rows:
#         f.write(row + "\n")

df = pd.read_csv(sales_file)
# print("데이터 준비 완료:", df.shape[0], "건")


# ----------------------
# groupby 가 하는 일
# ----------------------
#
# pandas 에서 가장 많이 쓰는 기능입니다.
# "OO 별로 묶어서 계산해줘"를 한 줄로 처리합니다
#

# [지금까지는 이렇게 했습니다]

#   dept_total = {}
#   dept_count = {}
#   for row in rows:
#       dept = row["지점"]
#       dept_total[dept] = dept_total.get(dept, 0) + row["매출액"]
#       dept_count[dept] = dept_count.get(dept, 0) + 1
#
#   for dept in dept_total:
#       avg = dept_total[dept] / dept_count[dept]
#       print(dept, avg)

# 15 줄 가까이 됩니다.

# [pandas 는 이렇게 씁니다]
# df.groupby("지점")["매출액"].mean()
# 이 한줄이 끝!


# [groupby의 3단계]
#
# 1) 쪼갠다    지점별로 데이터를 나눔
# 2) 계산한다   각 덩어리마다 합계나 평균을 냄
# 3) 합친다    결과를 하나의 표로 모음
#
# 우리는 이 세 단계를 한 줄로 시키는 겁니다.

# 먼저 매출액 열을 만들어 둡시다
df["매출액"] = df["수량"] * df["단가"]


print("[지점별 매출 합계]")
print(df.groupby("지점")["매출액"].sum())

print("\n[지점별 매출 평균]")
print(df.groupby("지점")["매출액"].mean().round(0))

print("\n[지점별 주문 건수]")
print(df.groupby("지점").size())

""" 읽는 법
    df.groupby("지점")["매출액"].sum()
        무엇으로    어느 열을   어떻게
        묶을까?    계산 할까?  계산 할까?

        "지점으로 묶어서, 매출액을, 더해라
"""


# -------------------------
# 자주 쓰는 계산 함수들
# -------------------------

# g = df.groupby("지점")["매출액"]

# print("합계 sum(): ", dict(g.sum()))
# print("평균 mean(): ", dict(g.mean().round(0)))
# print("개수 count(): ", dict(g.count()))
# print("최대 max(): ", dict(g.max()))
# print("최소 min(): ", dict(g.min()))

""" 자주 쓰는 것들
    sum()     합계
    mean()    평균
    count()   개수(빈 값 제외)
    size()    개수(빈 값 포함)
    max()     최대값
    min()     최소값
    median()  중앙값
    std()     표준편차
    nunique   서로 다른 값의 개수

    count()와 size()의 차이에 주의하세요.
    빈 값이 있을 때 결과가 달라 집니다.
"""


# --------------------------
# 한 번에 여러 계산하기 - agg
# ------------------------

# 합계도 보고 평균도 보고 건수도 보고 싶을 때가 많습니다.
# agg를 쓰면 한 번에 됩니다.

# result = df.groupby("지점")["매출액"].agg(["sum", "mean", "count", "max"])

# print(result)


""" agg 에 리스트로 넘기면 여러 계산을 한 번에 합니다.
    숫자가 1.452500e+06 처럼 나오는 것이 있죠?
    이건 지수 표기입니다. 1.4525 x 10의 6제곱, 즉 1,452,500 입니다.
    숫자가 크고 소수점이 있으면 pandas가 이렇게 줄여서 보여 줍니다.

    보기 불편하면 round()로 소수점을 없애면 됩니다.
"""

# 열 이름도 바꾸고 싶으면 이렇게 씁니다.

# 열 이름을 지정하는 방법
# result2 = df.groupby("지점").agg(
#     총매출=("매출액", "sum"),
#     평균매출=("매출액", "mean"),
#     건수=("매출액", "count"),
#     총수량=("수량", "sum"),
# )

# print(result2.round(0))

"""
    읽는 법
    
    총매출=("매출액", "sum")
    새열이름  어느 열   무슨계산

    이 방식이 결과를 보기 좋게 만듭니다. 실무에서 많이 씁니다.
"""


# ----------------------------
# 두 개 이상으로 묶기
# ----------------------------


# print("[지점 + 분류별 매출]")
# result = df.groupby(["지점", "분류"])["매출액"].sum()
# print(result)

"""
  리스트로 넘기면 두 단계로 묶입니다.
  강남의 가전, 강남의 주변기기, 홍대의 가전... 이런 식

  결과가 계단처럼 보이는데, 이걸 보기 좋게 표로 만들려면 
  피벗 테이블을 쓰면 됩니다.
"""

# 결과를 다시 DataFrame 으로 만들기
# print("[표 형태로 보기] reset_index()")
# result_df = df.groupby(["지점", "분류"])["매출액"].sum().reset_index()

# print(result_df)

# reset_index() 를 붙이면 묶음 기준이 다시 일반 열이 됩니다.
# 결과를 CSV 로 저장하거나 다시 필터링할 때 필요합니다.

# ------------------------------
# 실전 - 정렬해서 순위 보기
# ------------------------------

# [매출 1위 사원부터]
top = df.groupby("사원")["매출액"].sum().sort_values(ascending=False)
print(top)

# 상위 3명만
print(top.head(3))

# 가장 잘 팔린 상품
best = df.groupby("상품")["수량"].sum().sort_values(ascending=False)
print(best)
print("\n1등: ", best.index[0], "-", best.iloc[0], "개")

"""
groupby 결과도 정렬할 수 있습니다.
sort_values(ascending=False)를 붙이면 큰 순서로 나옵니다.

best.index[0] -> 맨 위 항목의 이름
best.iloc[0] -> 맨 위 항목의 값
"""


# -----------------------------------
# 빠진 값 찾기
# -----------------------------------
#
# 실무 데이터에는 빈 칸이 반드시 있습니다.
# 담당자가 입력을 안 했거나, 시스템 오류거나, 애초에 해당 없거나요.
#
# pandas는 빈 값을 NaN 으로 표시합니다.
# Not a Number 의 줄임말인데, "값 없음" 이라고 이해하면 됩니다.

# 열별 빠진 값 개수
# print(df.isnull().sum())

# 빠진 값이 있는 행만 보기
# print(df[df.isnull().any(axis=1)])


"""
df.isnull()               각 칸이 비었는지 True/False
df.isnull().sum()         열별로 몇 개나 비었는지
df.isnull().any(axis=1)   행마다 하나라도 비었는지

axis=1 은 "행 방향으로 확인" 이라는 뜻입니다.
axis=0 이면 열 방향입니다.  헷갈리면 둘 다 해보세요.
"""

# --------------------------------
# 빠진 값 채우기 - fillna
# --------------------------------
#
# 어떻게 채울지는 데이터의 성격에 따라 다릅니다.
# 정답이 없으니 상황에 맞게 판단해야 합니다.

# 방법 1) 0으로 채우기 - 수량이나 금액에 어울립니다
df_fill = df.copy()  # 원본을 지키려고 복사본을 만듭니다.
# df_fill["수량"] = df_fill["수량"].fillna(0)
# 수량을 0으로 채움
# print(df_fill[["상품", "수량"]].head(8))

# 방법 2) 평균으로 채우기 - 단가처럼 대표값이 있을 때
# avg_price = df["단가"].mean()
# df_fill["단가"] = df_fill["단가"].fillna(avg_price)
# print(f"\n 단가를 평균 {avg_price:,.0f}원으로 채움")


# 방법 3) 글자로 채우기 - 이름 같은 문자열 열
# df_fill["사원"] = df_fill["사원"].fillna("미지정")
# 사원을 '미지정'으로 채움
# print(df_fill[df_fill["사원"] == "미지정"][["주문일", "지점", "사원"]])

"""
어떻게 채울지 정하는 기준

수량, 건수      -> 0 이 자연스럽습니다.
금액, 점수      -> 평균이나 중앙값
이름, 분류      -> "미지정" 같은 글자
시계열 데이터    -> 앞의 값으로 (ffill)

주의: 함부로 채우면 안 됩니다.
"빠진 것" 자체가 정보일 수 있습니다.
왜 비었는지 먼저 확인하는 게 순서 입니다.
"""


# ---------------------------------------
# 빠진 값이 있는 행 지우기 - dropna
# ---------------------------------------

# print("원본: ", len(df), "건")

# 하나라도 빈 값이 있으면 그 행 삭제
# df_drop = df.dropna()
# print("dropna()후: ", len(df_drop), "건")

# 특정 열만 확인해서 삭제
# df_drop2 = df.dropna(subset=["수량"])
# print("수량이 빈 행만 삭제: ", len(df_drop2), "건")

"""
dropna() 옵션

df.dropna()   하나하도 비면 그 행 삭제
df.dropna(subset=["수량"])   수량이 빈 행만 삭제
df.dropna(how="all")   모든 칸이 비었을 때만 삭제
df.dropna(axis=1)      빈 값이 있는 '열' 삭제

삭제할지 채울지 판단하는 기준

빈 값이 전체의 5% 미만     -> 삭제해도 큰 영향 없음
빈 값이 많음 (30%이상)    -> 채우거나, 그 열을 아예 빼기
핵심 열(매출액 등)이 비었음  -> 그 행은 쓸 수 없으니 삭제

"""

# -------------------------------
# 중복 처리
# -------------------------------


# print("중복된 행 개수: ", df.duplicated().sum())

# 특정 열 기준으로 중복 확인
# print("사원 이름 중복 개수: ", df.duplicated(subset="사원").sum())

# 중복 제거
# df_unique = df.drop_duplicates(subset=["사원"])
# print("사원별 첫 주문만 남기면: ", len(df_unique), "건")
# print(df_unique[["사원", "지점"]])

"""
drop_duplicates 옵션

keep="first"     첫 번째만 남김 (기본값)
keep="last"      마지막만 남김
keep=False       중복된 것 전부 삭제 
"""


# ----------------------------------
# 날짜로 변환 하기
# ----------------------------------
#
# CSV에서 읽은 날짜는 그냥 문자열입니다.
# 날짜로 바꿔야 월별 집계나 요일별 분석을 할 수 있습니다.

# print("변환 전 자료형: ", df["주문일"].dtype)

df["주문일"] = pd.to_datetime(df["주문일"])
# print("변환 후 자료형: ", df["주문일"].dtype)
# 변환 후에는 이런 것들을 꺼낼 수 있습니다
# print(df["주문일"].head(3))


# 날짜에서 정보 꺼내기 - .dt를 붙입니다.
df["월"] = df["주문일"].dt.month
df["일"] = df["주문일"].dt.day
df["요일"] = df["주문일"].dt.day_name()

# 날짜에서 꺼낸 정보
# print(df[["주문일", "월", "일", "요일"]].head(5))

"""
.dt 로 꺼낼 수 있는 것들

.dt.year        연도
.dt.month       월
.dt.day         일
.dt.dayofweek   요일 (0=월요일, 6=일요일)
.dt.day_name()  요일 이름(Monday...)
.dt.hour        시
.dt.quarter     분기

왜 .dt 를 붙이나요?
문자열 열에 .str을 붙이는 것과 같은 원리입니다.
"이 열을 날짜로 취급해서 다뤄라"는 표시입니다.

"""

# ------------------------
# 요일을 한글로 바꾸기
# ------------------------

# 딕셔너리를 만들어 두고 map 으로 바꿉니다
week_kr = {
    "Monday": "월",
    "Tuesday": "화",
    "Wednesday": "수",
    "Thursday": "목",
    "Friday": "금",
    "Saturday": "토",
    "Sunday": "일",
}

df["요일한글"] = df["요일"].map(week_kr)
# print(df[["주문일", "요일", "요일한글"]].head(5))

"""
map()은 딕셔너리를 보고 값을 바꿔즈는 기능입니다.
df["열"].map({"옛값":"새값",...})
등급 매기기, 코드를 이름으로 바꾸기 등에 자주 씁니다.
"""

# -----------------------------
# 날짜로 집계하고 필터링하기
# -----------------------------

# 요일별 매출
# print(df.groupby("요일한글")["매출액"].sum())

# 날짜별 매출
# daily = df.groupby("주문일")["매출액"].sum()
# print(daily)

# 특정 날짜 이후만
# recent = df[df["주문일"] >= "2026-03-06"]
# print(recent[["주문일", "지점", "매출액"]])

"""
날짜로 바꿔두면 부등호 비교가 됩니다.

문자열 상태로도 비교가 되긴 하지만
날짜로 바꿔두는 게 안전하고 기능도 많습니다.
"""

# ---------------------------------------
# 엑셀 피벗 테이블과 같은 것
# ---------------------------------------
#
# groupby로 두 개를 묶으면 결과가 계단처럼 나옵니다.
# 이걸 가로세로 표로 만들어 주는 게 pivot_table입니다.
#
# 엑셀 피벗 테이블을 써 보셨다면 같은 개념입니다.

# groupby로 하면 계단모양
# print(df.groupby(["지점", "분류"])["매출액"].sum())

# pivot_table로 하면 표모양
pivot = df.pivot_table(
    index="지점",  # 세로축에 올 것
    columns="분류",  # 가로축에 올 것
    values="매출액",  # 계산할 값
    aggfunc="sum",  # 어떻게 계산할지
)
# print(pivot)

"""
읽는 법
index= 왼쪽 세로에 놓을 항목
columns= 위쪽 가로에 놓을 항목
vaules= 칸 안에 넣을 값
aggfunc= 계산 방법 (sum, mean, count 등)

보고서를 만들 때 이 형태가 훨씬 읽기 좋습니다.
"""

# 합계 행과 열 추가하기
# pivot2 = df.pivot_table(
#     index="지점",
#     columns="분류",
#     values="매출액",
#     aggfunc="sum",
#     margins=True,  # 합계 추가
#     margins_name="합계",  # 합계 칸 이름
# )
# print(pivot2)

# 빈 칸을 0으로 채우기
# pivot3 = df.pivot_table(
#     index="사원",
#     columns="분류",
#     values="수량",
#     aggfunc="sum",
#     fill_value=0,
# )
# print(pivot3)

# ---------------------------------
# CSV 로 저장
# ---------------------------------
#
# 계산한 결과를 파일로 남겨야 남에게 줄 수 있습니다.
#
# 여기서 앞서 배운 encoding이 다시 나옵니다.
# 엑셀에서 열 파일이면 반드시 utf-8-sig 로 저장하세요.

out1 = DATA / "지점별_매출.csv"

summary = (
    df.groupby("지점")
    .agg(
        총매출=("매출액", "sum"), 평균매출=("매출액", "mean"), 건수=("매출액", "count")
    )
    .round(0)
)

# summary.to_csv(out1, encoding="utf-8-sig")
# print(f"저장완료: {out1.name}")
# print(summary)

"""
to_csv 옵션
df.to_csv("파일.csv", encoding="utf-8-sig")
  엑셀에서 한글이 안 깨집니다. 이걸 기본으로 쓰세요.

df.to_csv("파일.csv", index=False)
  맨 왼쪽 인덱스 번호를 빼고 저장합니다
  groupby 결과가 아닌 일반 데이터는 보통 이걸 씁니다.
  
  df.to_csv("파일.csv", columns=["지점", "매출액"])
    원하는 열만 저장합니다.
"""

# 일반 데이터를 저장할 때는 index=False
# out2 = DATA / "정리된_주문내역.csv"
# df.to_csv(out2, encoding="utf-8-sig", index=False)
# print(f"저장 완료: {out2.name}")

# 저장한 파일 다시 읽어서 확인
# check = pd.read_csv(out2)
# print("다시 읽어보기: ", check.shape)

# -----------------------------
# 엑셀로 저장
# -----------------------------
# 엑셀 파일 (.xlsx)로 저장하려면 openpyxl 이 필요합니다.
# 여러 시트로 나눠 저장할 수도 있습니다.

try:
    out3 = DATA / "매출보고서.xlsx"

    # 여러 시트에 나눠 저장
    with pd.ExcelWriter(out3) as writer:
        df.to_excel(writer, sheet_name="전체내역", index=False)
        summary.to_excel(writer, sheet_name="지점별요약")
        pivot.to_excel(writer, sheet_name="피벗")

    print(f"저장 완료:{out3.name}")
    print("시트 3개: 전체내역, 지점별요약, 피벗")
except ImportError:
    print("openpyxl 이 설치되어 있지 않습니다.")
    print("터미널에서 다음을 실행하세요.")
    print("pip install openpyxl")

"""
엑셀 저장의 장점
  - 여러 시트로 나눌 수 있습니다
  - 받는 사람이 바로 열어봅니다
  - 서식이 유지됩니다

CSV의 장점
  - 가볍고 빠릅니다
  - 어떤 프로그램에서든 열립니다

보고용이면 엑셀, 데이터 주고받기용이면 CSV를 쓰세요.
"""


# -------------------------------------
# 실전 - 보고서를 만드는 전체 흐름
# -------------------------------------
#
# 지금까지 배운 것을 순서대로 이어보면 이렇습니다.

# 1) 읽기
report = pd.read_csv(sales_file)

# 2) 정리 - 빠진 값 처리
report["수량"] = report["수량"].fillna(0)
report["단가"] = report["단가"].fillna(report["단가"].mean())
report["사원"] = report["사원"].fillna("미지정")

# 3) 계산 - 새 열 만들기
report["매출액"] = report["수량"] * report["단가"]
report["주문일"] = pd.to_datetime(report["주문일"])

# 4) 집계
by_branch = (
    report.groupby("지점")
    .agg(
        건수=("매출액", "count"),
        총매출=("매출액", "sum"),
        평균=("매출액", "mean"),
    )
    .round(0)
    .sort_values("총매출", ascending=False)
)

# 5) 출력
print("[지점별 실적]")
print(by_branch)

# 6) wjwkd
final = DATA / "최종 보고서.csv"
by_branch.to_csv(final, encoding="utf-8-sig")
print(f"\n저장 완료: {final.name}")

"""
이게 실무에서 데이터를 다루는 기본 흐름입니다.

읽기 -> 정리 -> 계산 -> 집계 -> 출력 -> 저장
코드로는 20줄 정도 입니다.
csv 모듈로 하면 100줄이 넘어갑니다
"""

#   [groupby]
#
#     df.groupby("열")["값열"].sum()          묶어서 합계
#     df.groupby(["열1","열2"])["값"].mean()   두 기준으로
#     df.groupby("열").agg(
#         이름=("값열", "sum"),
#     )                                      여러 계산 한 번에
#     .reset_index()                          결과를 표로 되돌리기
#
#
#   [빠진 값]
#
#     df.isnull().sum()            열별 빈 값 개수
#     df["열"].fillna(0)            0으로 채우기
#     df["열"].fillna(df["열"].mean())   평균으로 채우기
#     df.dropna()                  빈 값 있는 행 삭제
#     df.dropna(subset=["열"])      특정 열만 확인해서 삭제
#     df.drop_duplicates()         중복 제거
#
#
#   [날짜]
#
#     pd.to_datetime(df["열"])     문자열을 날짜로
#     df["열"].dt.month            월 꺼내기
#     df["열"].dt.day_name()       요일 이름
#     df["열"].map({...})          값 바꾸기
#
#
#   [피벗]
#
#     df.pivot_table(
#         index="세로", columns="가로",
#         values="값", aggfunc="sum",
#         margins=True, fill_value=0,
#     )
#
#
#   [저장]
#
#     df.to_csv("파일.csv", encoding="utf-8-sig", index=False)
#     df.to_excel("파일.xlsx", index=False)
#
#
#   [반드시 기억할 5가지]
#     1. groupby 는 "무엇으로 묶어서, 어느 열을, 어떻게 계산할지"
#     2. 빈 값은 채울지 지울지 데이터 성격을 보고 판단한다
#     3. 날짜는 to_datetime 으로 바꿔야 .dt 기능을 쓸 수 있다
#     4. 저장할 때 encoding="utf-8-sig" 를 잊지 말자
#     5. 실무 흐름은 읽기-정리-계산-집계-출력-저장